...
export function formatArea(
  ...
  const squarePoints = points.length === 2
    ? [{ x: a.x, y: a.y }, { x: b.x, y: a.y }, { x: b.x, y: b.y }, { x: a.x, y: b.y }]
    : points;
  const areaInPoints = Math.abs(squarePoints.reduce((sum, point, index) => {
...
function cloudPath(a: { x: number; y: number }, b: { x: number; y: number }, radius?: number) {
  ...
  const bump = Math.max(4, Math.min(40, radius ?? Math.max(6, Math.min(18, Math.min(width, height) / 5))));
  const segments: string[] = [`M ${left} ${top}`];
  const edge = (length: number, horizontal: boolean, forward: boolean, startX: number, startY: number) => {
...
function LineEndingVisual({
  ...
  if (style === 'open-arrow') {
    return <path d={`M ${wingA.x} ${wingA.y} L ${tip.x} ${tip.y} L ${wingB.x} ${wingB.y}`} fill="none" stroke={color} strokeWidth={width} strokeLinecap="round" strokeLinejoin="round" opacity={opacity} />;
  }
  if (style === 'closed-arrow' || style === 'filled-arrow') {
    return <path d={`M ${wingA.x} ${wingA.y} L ${tip.x} ${tip.y} L ${wingB.x} ${wingB.y} Z`} fill={style === 'filled-arrow' ? color : 'none'} stroke={color} strokeWidth={width} strokeLinejoin="round" opacity={opacity} />;
  }
  if (style === 'dot') return <circle cx={tip.x} cy={tip.y} r={Math.max(3, width * 1.5)} fill={color} opacity={opacity} />;
  if (style === 'square') {
    const size = Math.max(6, width * 3);
    const angle = Math.atan2(unit.y, unit.x) * 180 / Math.PI;
    return <rect x={tip.x - size / 2} y={tip.y - size / 2} width={size} height={size} fill={color} opacity={opacity} transform={`rotate(${angle} ${tip.x} ${tip.y})`} />;
  }
  const capHalf = Math.max(5, width * 2.5);
  const slashDirection = style === 'slash'
    ? { x: normal.x + unit.x * 0.7, y: normal.y + unit.y * 0.7 }
    : normal;
  return (
    <line
...
function BoxBoundary({
  ...
  if (markup.lineStyle === 'cloud') return <path {...props} d={cloudPath(a, b, markup.cloudRadius)} />;
  if (shape === 'ellipse') return <ellipse {...props} cx={left + width / 2} cy={top + height / 2} rx={width / 2} ry={height / 2} />;
  return <rect {...props} x={left} y={top} width={width} height={height} rx={shape === 'rounded' ? Math.min(12, height / 3) : 0} />;
}
...
function MarkupVisual({
  ...
  const defaultBold = markup.type === 'cloud-plus' || markup.type === 'dimension' || markup.type === 'area' || markup.type === 'radius' || markup.type === 'diameter' || markup.type === 'arc';
  ...
  if (markup.type === 'text') {
    if (points.length >= 2 && (width > 2 || height > 2)) {
      ...
      const textX = alignment === 'center' ? left + width / 2 : alignment === 'right' ? left + width - padding : left + padding;
      return (
        <g opacity={markup.opacity}>
          ...
          {!hideText && (
            <text
              ...
              textAnchor={alignment === 'center' ? 'middle' : alignment === 'right' ? 'end' : 'start'}
              fill={textColor}
            ...
            >
              {lines.slice(0, maxLines).map((line, index) => (
                <tspan key={`${line}-${index}`} x={textX} dy={index === 0 ? 0 : markup.fontSize * 1.25}>
                  {line}
                </tspan>
              ))}
            </text>
    ...
    return (
      <text
        ...
        fontWeight={fontWeight}
        fontStyle={fontStyle}
        style={{ paintOrder: 'stroke', stroke: 'white', strokeWidth: Math.max(1, markup.fontSize / 14) }}
      >
        {markup.text || 'Text'}
      </text>
    );
  ...
  if (markup.type === 'pen' || markup.type === 'highlight') {
    return (
      <polyline
        ...
        strokeWidth={markup.type === 'highlight' ? Math.max(10, markup.strokeWidth) : markup.strokeWidth}
  ...
  if (markup.type === 'line' || markup.type === 'arrow') {
    return (
      <g>
        ...
        <LineEndingVisual style={defaultStartEnding(markup)} from={b} tip={a} color={markup.stroke} width={markup.strokeWidth} opacity={markup.opacity} />
        <LineEndingVisual style={defaultEndEnding(markup)} from={a} tip={b} color={markup.stroke} width={markup.strokeWidth} opacity={markup.opacity} />
      </g>
    );
  }
  if (markup.type === 'rectangle') {
    return <rect {...common} x={left} y={top} width={width} height={height} fill={markup.fill} fillOpacity={markup.fillOpacity ?? 1} />;
  }
  ...
  if (markup.type === 'cloud') {
    return <path {...common} d={cloudPath(a, b, markup.cloudRadius)} fill={markup.fill} fillOpacity={markup.fillOpacity ?? 1} />;
  }
  if (markup.type === 'area' && points.length >= 2) {
    const polygon = points.length === 2
      ? [a, { x: b.x, y: a.y }, b, { x: a.x, y: b.y }]
      : points;
    const label = markup.text || (calibration ? formatAreaValue(
      (() => {
        const source = markup.points.length === 2
          ? [{ x: markup.points[0].x, y: markup.points[0].y }, { x: markup.points[1].x, y: markup.points[0].y }, { x: markup.points[1].x, y: markup.points[1].y }, { x: markup.points[0].x, y: markup.points[1].y }]
          : markup.points;
        return Math.abs(source.reduce((sum, point, index) => {
          const next = source[(index + 1) % source.length];
          return sum + (point.x * page.w) * (next.y * page.h) - (next.x * page.w) * (point.y * page.h);
        }, 0)) / 2 * calibration.unitsPerPoint ** 2;
      })(),
      calibration,
  ...
  if (markup.type === 'callout' && points.length >= 3) {
    ...
    const edgeScale = Math.min(
      1,
      1 / Math.max(Math.abs(dx) / Math.max(1, boxWidth / 2), Math.abs(dy) / Math.max(1, boxHeight / 2)),
    );
    ...
    const padding = Math.max(4, markup.fontSize * 0.35);
    const textX = alignment === 'left' ? boxLeft + padding : alignment === 'right' ? boxLeft + boxWidth - padding : cx;
    return (
      <g opacity={markup.opacity}>
        <polyline
          fill="none"
        ...
        />
        <LineEndingVisual style={markup.leaderEnding ?? 'filled-arrow'} from={elbow} tip={tip} color={leaderStroke} width={leaderWidth} opacity={leaderOpacity} />
  ...
  if (markup.type === 'cloud-plus' && points.length >= 3) {
    ...
    return (
      <g opacity={markup.opacity}>
        <path {...common} d={cloudPath(cloudA, cloudB, markup.cloudRadius)} fill={markup.fill} fillOpacity={markup.fillOpacity ?? 1} />
        <polyline
          fill="none"
          stroke={leaderStroke}
          strokeWidth={leaderWidth}
        ...
        />
        <LineEndingVisual style={markup.leaderEnding ?? 'filled-arrow'} from={elbow} tip={arrowTip} color={leaderStroke} width={leaderWidth} opacity={leaderOpacity} />
        <text
          x={label.x}
          y={label.y - 5}
  ...
  if ((markup.type === 'radius' || markup.type === 'diameter') && points.length >= 2) {
    ...
    const radius = markup.type === 'radius' ? Math.hypot(b.x - a.x, b.y - a.y) : Math.hypot(b.x - a.x, b.y - a.y) / 2;
    const edge = markup.type === 'radius' ? b : a;
    const labelValue = radius * (calibration?.unitsPerPoint ?? 0);
    const label = markup.text || (calibration ? `${markup.type === 'radius' ? 'R' : 'Ø'} ${formatLengthValue(labelValue, calibration, markup.measurementUnit, markup.fractionDenominator)}` : 'Not calibrated');
    ...
    return (
      <g>
        ...
        {markup.type === 'diameter' && (
          <>
            <LineEndingVisual style={defaultStartEnding(markup)} from={b} tip={a} color={markup.stroke} width={markup.strokeWidth} opacity={markup.opacity} size={markup.arrowSize} />
            <LineEndingVisual style={defaultEndEnding(markup)} from={a} tip={b} color={markup.stroke} width={markup.strokeWidth} opacity={markup.opacity} size={markup.arrowSize} />
          </>
        )}
        <rect x={edge.x + 4} y={edge.y - markup.fontSize - 5} width={labelWidth} height={markup.fontSize * 1.35} rx={2} fill="white" opacity={0.94 * markup.opacity} />
        <text x={edge.x + 8} y={edge.y - 7} fill={textColor} opacity={markup.opacity} fontSize={markup.fontSize} fontFamily={fontFamily} fontWeight={fontWeight} fontStyle={fontStyle}>{label}</text>
      </g>
    );
  }
  if (markup.type === 'arc' && points.length >= 3) {
    ...
    const label = markup.text || (calibration ? `Arc ${formatLengthValue(geometry.radius * geometry.sweep * calibration.unitsPerPoint, calibration, markup.measurementUnit, markup.fractionDenominator)}` : 'Not calibrated');
    const labelWidth = Math.max(40, label.length * markup.fontSize * 0.62 + 8);
    return (
      <g>
        <path {...common} d={arcPath(geometry, points[0], points[2])} />
        <circle cx={points[0].x} cy={points[0].y} r={Math.max(2, markup.strokeWidth)} fill={markup.stroke} opacity={markup.opacity} />
        <circle cx={points[2].x} cy={points[2].y} r={Math.max(2, markup.strokeWidth)} fill={markup.stroke} opacity={markup.opacity} />
        <rect x={geometry.midpoint.x - labelWidth / 2} y={geometry.midpoint.y - markup.fontSize * 0.7} width={labelWidth} height={markup.fontSize * 1.4} rx={2} fill="white" opacity={0.94 * markup.opacity} />
        <text x={geometry.midpoint.x} y={geometry.midpoint.y} textAnchor="middle" dominantBaseline="middle" fill={textColor} opacity={markup.opacity} fontSize={markup.fontSize} fontFamily={fontFamily} fontWeight={fontWeight} fontStyle={fontStyle}>{label}</text>
      </g>
    );
  }
  if (markup.type === 'dimension') {
    ...
    const label = markup.text || formatMeasurement(markup.points, page, calibration, markup.measurementUnit, markup.fractionDenominator);
    ...
    return (
      <g>
        {(markup.witnessLines ?? true) && (
          <>
            <line {...common} strokeWidth={dimensionStroke} x1={a.x - labelNormal.x * 4} y1={a.y - labelNormal.y * 4} x2={dimensionA.x + labelNormal.x * extension} y2={dimensionA.y + labelNormal.y * extension} />
            <line {...common} strokeWidth={dimensionStroke} x1={b.x - labelNormal.x * 4} y1={b.y - labelNormal.y * 4} x2={dimensionB.x + labelNormal.x * extension} y2={dimensionB.y + labelNormal.y * extension} />
          </>
        )}
        <line {...common} strokeWidth={dimensionStroke} x1={dimensionA.x} y1={dimensionA.y} x2={dimensionB.x} y2={dimensionB.y} />
        <LineEndingVisual style={defaultStartEnding(markup)} from={dimensionB} tip={dimensionA} color={markup.stroke} width={dimensionStroke} opacity={markup.opacity} size={markup.arrowSize} />
        <LineEndingVisual style={defaultEndEnding(markup)} from={dimensionA} tip={dimensionB} color={markup.stroke} width={dimensionStroke} opacity={markup.opacity} size={markup.arrowSize} />
        <rect x={mx - labelWidth / 2} y={my - markup.fontSize * 0.7} width={labelWidth} height={markup.fontSize * 1.4} rx={2} fill="white" opacity={0.96 * markup.opacity} />
        <text x={mx} y={my} textAnchor="middle" dominantBaseline="middle" fill={textColor} opacity={markup.opacity} fontSize={markup.fontSize} fontFamily={fontFamily} fontWeight={fontWeight} fontStyle={fontStyle}>
          {label}
        </text>
      </g>
    );
...
function MarkupHitTarget({
  ...
  const textTarget = (x: number, y: number, text: string, anchor: 'start' | 'end' | 'middle' = 'start') => {
    ...
    const targetX = anchor === 'end' ? x - targetWidth : anchor === 'middle' ? x - targetWidth / 2 : x;
    return <rect {...enclosedTarget} x={targetX} y={y - targetHeight} width={targetWidth} height={targetHeight} rx={2 / scale} />;
  };

  if (markup.type === 'pen' || markup.type === 'highlight') {
    return <polyline {...strokeTarget} points={points.map((point) => `${point.x},${point.y}`).join(' ')} />;
  }
  if (markup.type === 'line' || markup.type === 'arrow') {
  ...
  if (markup.type === 'text') {
    if (points.length >= 2 && (width > 2 || height > 2)) {
      return <rect {...enclosedTarget} x={left} y={top} width={Math.max(width, 12 / scale)} height={Math.max(height, 12 / scale)} />;
    }
  ...
  if (markup.type === 'ellipse') {
    return <ellipse {...enclosedTarget} cx={left + width / 2} cy={top + height / 2} rx={width / 2} ry={height / 2} />;
  }
  if (markup.type === 'cloud') {
  ...
  if (markup.type === 'area' && points.length >= 2) {
    const polygon = points.length === 2
      ? [a, { x: b.x, y: a.y }, b, { x: a.x, y: b.y }]
      : points;
    return <polygon {...enclosedTarget} points={polygon.map((point) => `${point.x},${point.y}`).join(' ')} />;
  }
  if (markup.type === 'callout' && points.length >= 3) {
    const tip = points[0];
    const boxA = points[1];
    ...
    return (
      <g>
        <polyline {...strokeTarget} points={`${attach.x},${attach.y} ${elbow.x},${elbow.y} ${tip.x},${tip.y}`} />
  ...
  if (markup.type === 'cloud-plus' && points.length >= 3) {
    ...
    return (
      <g>
        ...
        <polyline {...strokeTarget} points={`${label.x},${label.y} ${elbow.x},${elbow.y} ${arrowTip.x},${arrowTip.y}`} />
  ...
  if (markup.type === 'arc' && points.length >= 3) {
    ...
    return (
      <g>
        ...
        {textTarget(geometry.midpoint.x, geometry.midpoint.y + markup.fontSize * 0.55, markup.text || 'Arc', 'middle')}
      </g>
    );
  }
  if (markup.type === 'dimension') {
    ...
    return (
      <g>
        {(markup.witnessLines ?? true) && (
          <>
            <line {...strokeTarget} x1={a.x} y1={a.y} x2={dimensionA.x + labelNormal.x * extension} y2={dimensionA.y + labelNormal.y * extension} />
            <line {...strokeTarget} x1={b.x} y1={b.y} x2={dimensionB.x + labelNormal.x * extension} y2={dimensionB.y + labelNormal.y * extension} />
          </>
        )}
        <line {...strokeTarget} x1={dimensionA.x} y1={dimensionA.y} x2={dimensionB.x} y2={dimensionB.y} />
        {textTarget(midpoint.x, midpoint.y + markup.fontSize * 0.55, markup.text || 'Dimension', 'middle')}
      </g>
    );
  }
  return null;
...
function EditableMarkup({
  ...
  const onPointerMove = (e: React.PointerEvent<SVGGElement>) => {
    ...
    if (snappingEnabled && snapPoints.length > 0) {
      ...
      for (const source of nextPoints) {
        ...
        if (!target) continue;
        const distance = Math.hypot((target.x - source.x) * page.w * viewScale, (target.y - source.y) * page.h * viewScale);
      ...
      if (best) {
        ...
        if (shifted.every((point) => point.x >= 0 && point.x <= 1 && point.y >= 0 && point.y <= 1)) {
          nextPoints = shifted;
    ...
    <circle
      ...
      onPointerMove={(e) => {
        ...
        if (e.shiftKey) {
          ...
          if ((markup.type === 'line' || markup.type === 'arrow' || markup.type === 'dimension') && next.length >= 2) {
            ...
            (markup.type === 'text' || markup.type === 'rectangle' || markup.type === 'ellipse' || markup.type === 'cloud' || markup.type === 'area')
            && next.length >= 2
          ) {
      ...
      onPointerCancel={() => { handleRef.current = null; onSnapPointChange(null); }}
    />
  );
  ...
  const inlineEditor = () => {
    ...
    if (cloudPlus) {
      ...
      const editorWidth = Math.max(100, Math.min(220, (markup.text.length + 4) * markup.fontSize * 0.62));
      return (
        <foreignObject
  ...
  return (
    <g
      style={{ pointerEvents: canEdit ? 'visiblePainted' : 'none', cursor: canEdit ? 'move' : 'default' }}
      ...
      onPointerCancel={() => { dragRef.current = null; onSnapPointChange(null); }}
      onDoubleClick={(e) => {
        if (markup.type !== 'text' && markup.type !== 'callout' && markup.type !== 'cloud-plus') return;
        e.preventDefault();
        e.stopPropagation();
        selectMarkup(markup.id);
        setEditingText(true);
    ...
    >
      <defs>
        <marker id={`arrow-${markup.id}`} markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
          <path d="M 0 0 L 8 4 L 0 8 z" fill={markup.stroke} />
        </marker>
      </defs>
      <MarkupHitTarget markup={markup} page={page} viewScale={viewScale} active={hovered && !selected && canEdit} />
      ...
      {selected && (
        <>
          <rect
            ...
            pointerEvents="none"
          />
          {markup.type !== 'pen' && markup.type !== 'highlight' && markup.points.map((point, index) => resizeHandle(index, point))}
        </>
      )}
    </g>
  );
...
export function MarkupLayer({
  ...
  return (
    ...
    >
      <defs>
        <marker id="arrow-draft" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
