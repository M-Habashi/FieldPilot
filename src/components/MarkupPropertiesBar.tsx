...
function MarkupControls({
  ...
  const supportsCloudSize =
    markup.type === 'cloud' ||
    markup.type === 'cloud-plus' ||
    (markup.type === 'callout' && markup.lineStyle === 'cloud');
  const filled = markup.fill !== 'transparent' && markup.fill !== 'none';
  const fillColor =
    filled && /^#[0-9a-f]{6}/i.test(markup.fill) ? markup.fill.slice(0, 7) : '#ffffff';
    ...
    <>
      ...
      <label className="flex shrink-0 items-center gap-1.5">
        ...
        <input
          ...
          onChange={(event) =>
            updateMarkup(markup.id, { strokeWidth: Math.max(0, Number(event.target.value) || 0) })
          }
        />
      </label>
      ...
      {supportsLineStyle && (
        <SelectField
          ...
          options={lineStyles}
          onChange={(lineStyle) =>
            updateMarkup(markup.id, { lineStyle: lineStyle as MarkupLineStyle })
          }
        />
      )}
      ...
      {hasEndings && (
        <>
          <SelectField
            ...
            value={
              markup.startEnding ??
              (markup.type === 'dimension' || markup.type === 'diameter' ? 'filled-arrow' : 'none')
            }
            ariaLabel="Start line ending"
            options={LINE_ENDING_OPTIONS}
            onChange={(startEnding) =>
              updateMarkup(markup.id, { startEnding: startEnding as MarkupLineEnding })
            }
          />
          <SelectField
            label="End"
            value={
              markup.endEnding ??
              (markup.type === 'arrow' || markup.type === 'dimension' || markup.type === 'diameter'
                ? 'filled-arrow'
                : 'none')
            }
            ...
            options={LINE_ENDING_OPTIONS}
            onChange={(endEnding) =>
              updateMarkup(markup.id, { endEnding: endEnding as MarkupLineEnding })
            }
          />
        </>
      )}
          ...
          <input
            ...
            onChange={(event) =>
              updateMarkup(markup.id, {
                cloudRadius: Math.max(4, Math.min(40, Number(event.target.value) || 10)),
              })
            }
          />
        </label>
        ...
        <>
          <SelectField
            ...
            onChange={(measurementUnit) =>
              updateMarkup(markup.id, {
                measurementUnit: measurementUnit as MeasurementDisplayUnit,
              })
            }
          />
          {markup.type !== 'area' && (
            <SelectField
              ...
              options={[1, 2, 4, 8, 16].map((value) => ({
                value: String(value),
                label: value === 1 ? 'Whole' : `1/${value}`,
              }))}
              onChange={(fractionDenominator) =>
                updateMarkup(markup.id, {
                  fractionDenominator: Number(fractionDenominator) as FractionDenominator,
                })
              }
              width="w-20"
            />
        ...
        <>
          ...
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              onChange={(event) =>
                updateMarkup(markup.id, {
                  extensionOffset: Math.max(0, Math.min(500, Number(event.target.value) || 0)),
                })
              }
            />
          </label>
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              onChange={(event) =>
                updateMarkup(markup.id, {
                  extensionLength: Math.max(0, Math.min(500, Number(event.target.value) || 0)),
                })
              }
            />
          </label>
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              onChange={(event) =>
                updateMarkup(markup.id, {
                  arrowSize: Math.max(2, Math.min(60, Number(event.target.value) || 2)),
                })
              }
            />
          </label>
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <label className="flex cursor-pointer items-center gap-1.5">
              <input
                ...
                onChange={(event) =>
                  updateMarkup(markup.id, {
                    fill: event.target.checked ? '#ffffff' : 'transparent',
                  })
                }
              />
        ...
        <>
          ...
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              className="h-6 w-7 cursor-pointer rounded-xs border border-line-strong bg-transparent p-0.5"
              onInput={(event) =>
                updateMarkup(markup.id, { leaderStroke: event.currentTarget.value })
              }
            />
          </label>
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              onChange={(event) =>
                updateMarkup(markup.id, {
                  leaderStrokeWidth: Math.max(0.5, Number(event.target.value) || 0.5),
                })
              }
            />
          </label>
          ...
          <SelectField
            ...
            onChange={(leaderLineStyle) =>
              updateMarkup(markup.id, { leaderLineStyle: leaderLineStyle as MarkupLineStyle })
            }
          />
          <SelectField
            label="Tip"
            ...
            options={LINE_ENDING_OPTIONS}
            onChange={(leaderEnding) =>
              updateMarkup(markup.id, { leaderEnding: leaderEnding as MarkupLineEnding })
            }
          />
        </>
      )}
        ...
        <>
          ...
          <label className="flex shrink-0 items-center gap-1.5">
            ...
            <input
              ...
              className="h-7 w-14 rounded-xs border border-line bg-surface2 px-2 font-mono text-xs text-t1"
              onChange={(event) =>
                updateMarkup(markup.id, { fontSize: Math.max(8, Number(event.target.value) || 8) })
              }
            />
          </label>
          <button
          ...
          <SelectField
            ...
            width="w-20"
            onChange={(textAlign) =>
              updateMarkup(markup.id, { textAlign: textAlign as Markup['textAlign'] })
            }
          />
        </>
      )}
      ...
      <div className="ml-auto flex shrink-0 items-center border-l border-line pl-2">
        ...
        <Button
          variant="text"
          size="iconXs"
          aria-label="Close markup properties"
          title="Close"
          onClick={() => selectMarkup(null)}
        >
          <X />
        </Button>
      </div>
