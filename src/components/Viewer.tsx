...
function constrainedDrawPoint(tool: MarkupTool, start: MarkupPoint, point: MarkupPoint, page: PageBase): MarkupPoint {
  if (
    tool === 'line'
    || tool === 'arrow'
    || tool === 'dimension'
  ...
  if (tool === 'text' || tool === 'rectangle' || tool === 'ellipse' || tool === 'cloud' || tool === 'cloud-plus' || tool === 'area') {
...
export function Viewer({ doc }: { doc: PDFDocumentProxy }) {
  ...
  const rasterSnapPoint = (point: MarkupPoint): MarkupPoint | null => {
    ...
    const radiusX = Math.max(2, Math.ceil((12 / Math.max(0.1, pageBase.w * view.scale)) * canvas.width));
    const radiusY = Math.max(2, Math.ceil((12 / Math.max(0.1, pageBase.h * view.scale)) * canvas.height));
  ...
  const resolveSnapPoint = (point: MarkupPoint, allowRasterFallback = true): MarkupPoint => {
    ...
    const snapped = vectorPoint ?? (allowRasterFallback && planSnapPoints.length === 0 ? rasterSnapPoint(point) : null);
    ...
    if (!middle && markupTool && markupTool !== 'select' && !addPinMode) {
      ...
      if (markupTool === 'area' && pageBase) {
        ...
        if (drawing?.tool === 'area') {
          ...
          if (drawing.points.length >= 3 && areaDistance(nextPoint, drawing.points[0], pageBase) < 12) {
            finishArea();
            return;
          }
      ...
      if (markupTool === 'arc') {
        ...
        drawRef.current = { tool: 'arc', start: point, end: point, points: [point], committedPoints: [point] };
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drawing) {
      const rawPoint = normalizedPoint(e.clientX, e.clientY);
      if (!rawPoint) return;
      const snapThisPoint = drawing.tool !== 'pen' && drawing.tool !== 'highlight' && drawing.tool !== 'callout';
      ...
      const shift = e.shiftKey || shiftPressedRef.current;
      const origin = drawing.tool === 'area'
        ? drawing.points[drawing.points.length - 1] ?? drawing.start
        : drawing.start;
      const point = shift && pageBase && drawing.tool === 'area'
      ...
      drawing.end = point;
      if (drawing.tool === 'area') {
        const preview = pageBase && drawing.points.length > 0 && areaDistance(point, origin, pageBase) >= 3
  ...
  const onPointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drawing) {
      if (drawing.tool === 'area') {
        if (pageBase && drawing.points.length === 1 && areaDistance(drawing.start, drawing.end, pageBase) >= 3) {
    ...
    >
      ...
      <div
        className={cn(
          ...
          'transition-[opacity,transform] duration-(--fp-dur-med) ease-(--fp-ease)',
          markupTool && markupTool !== 'select' ? 'opacity-100 translate-y-0' : '-translate-y-3 opacity-0',
        )}
      >
