...
export function arcGeometry(points: Point2D[]): ArcGeometry | null {
  ...
  const denominator = 2 * (a.x * (through.y - c.y) + through.x * (c.y - a.y) + c.x * (a.y - through.y));
  ...
  const ccw = (a.x - center.x) * (through.y - center.y) - (a.y - center.y) * (through.x - center.x) > 0;
  ...
  return {
    ...
    midpoint: { x: center.x + radius * Math.cos(midpointAngle), y: center.y + radius * Math.sin(midpointAngle) },
  };
}
