...
function cloudPath(ctx: CanvasRenderingContext2D, a: { x: number; y: number }, b: { x: number; y: number }, radius?: number) {
...
function fillRectWithOpacity(ctx: CanvasRenderingContext2D, markup: Markup, x: number, y: number, width: number, height: number) {
  if (!fillEnabled(markup.fill)) return;
  const alpha = ctx.globalAlpha;
  ctx.globalAlpha = alpha * (markup.fillOpacity ?? 1);
  ctx.fillStyle = markup.fill;
...
function formatMeasurement(markup: Markup, page: PageSize, calibration: PageCalibration | undefined) {
  return formatLengthBetween(markup.points, page, calibration, markup.measurementUnit, markup.fractionDenominator);
}
...
function formatArea(markup: Markup, page: PageSize, calibration: PageCalibration | undefined) {
  ...
  const points = markup.points.length === 2
    ? [{ x: a.x, y: a.y }, { x: b.x, y: a.y }, { x: b.x, y: b.y }, { x: a.x, y: b.y }]
    : markup.points;
  const areaInPoints = Math.abs(points.reduce((sum, point, index) => {
...
function drawMarkup(
  ...
  if (markup.type === 'text') {
    if (points.length >= 2 && (width > 2 || height > 2)) {
      ...
      const textX = ctx.textAlign === 'center' ? left + width / 2 : ctx.textAlign === 'right' ? left + width - padding : left + padding;
    ...
    ctx.lineWidth = markup.type === 'highlight' ? Math.max(10, markup.strokeWidth) : markup.strokeWidth;
    ...
    drawLineEnding(ctx, markup.leaderEnding ?? 'filled-arrow', elbow, tip, leaderStroke, leaderWidth);
    ctx.restore();
    ...
    ctx.textBaseline = 'middle';
    const padding = Math.max(4, markup.fontSize * 0.35);
    const textX = ctx.textAlign === 'left' ? boxLeft + padding : ctx.textAlign === 'right' ? boxLeft + boxWidth - padding : center.x;
    ...
    drawLineEnding(ctx, markup.leaderEnding ?? 'filled-arrow', elbow, arrowTip, leaderStroke, leaderWidth);
    ctx.restore();
    ctx.fillStyle = textColor;
    ctx.font = font;
    ...
    if (markup.witnessLines ?? true) {
      strokePath(ctx, [
        ...
        { x: dimensionA.x + labelNormal.x * extension, y: dimensionA.y + labelNormal.y * extension },
      ]);
      strokePath(ctx, [
        { x: b.x - labelNormal.x * 4, y: b.y - labelNormal.y * 4 },
        { x: dimensionB.x + labelNormal.x * extension, y: dimensionB.y + labelNormal.y * extension },
      ]);
    }
    strokePath(ctx, [dimensionA, dimensionB]);
    drawLineEnding(ctx, defaultStartEnding(markup), dimensionB, dimensionA, markup.stroke, ctx.lineWidth, markup.arrowSize);
    drawLineEnding(ctx, defaultEndEnding(markup), dimensionA, dimensionB, markup.stroke, ctx.lineWidth, markup.arrowSize);
    ctx.font = font;
    const labelWidth = Math.max(38, ctx.measureText(label).width + 8);
    ctx.fillStyle = 'white';
    ctx.fillRect(midpoint.x - labelWidth / 2, midpoint.y - markup.fontSize * 0.7, labelWidth, markup.fontSize * 1.4);
    ctx.fillStyle = textColor;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ...
    if (markup.type === 'diameter') {
      drawLineEnding(ctx, defaultStartEnding(markup), b, a, markup.stroke, markup.strokeWidth, markup.arrowSize);
      drawLineEnding(ctx, defaultEndEnding(markup), a, b, markup.stroke, markup.strokeWidth, markup.arrowSize);
    }
    const label = markup.text || (calibration ? `${markup.type === 'radius' ? 'R' : 'Ø'} ${formatLengthValue(radius * calibration.unitsPerPoint, calibration, markup.measurementUnit, markup.fractionDenominator)}` : 'Not calibrated');
    ctx.fillStyle = textColor;
    ctx.font = font;
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';
    ...
    if (geometry) {
      ctx.beginPath();
      ctx.arc(geometry.center.x, geometry.center.y, geometry.radius, geometry.startAngle, geometry.endAngle, geometry.sweepFlag === 0);
      ctx.stroke();
...
function canvasBlob(canvas: HTMLCanvasElement) {
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Could not render PDF markups.')), 'image/png');
  });
}

function rotateOverlay(displayCanvas: HTMLCanvasElement, page: PageSize, scale: number, angle: number) {
...
export async function createAnnotatedPdf(sourcePdf: Uint8Array, options: ExportOptions) {
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    const displayPage = angle === 90 || angle === 270 ? { w: size.height, h: size.width } : { w: size.width, h: size.height };
    ...
    for (const markup of pageMarkups) drawMarkup(ctx, markup, displayPage, options.calibrations[pageNumber]);
    const overlayCanvas = rotateOverlay(displayCanvas, { w: size.width, h: size.height }, scale, angle);
