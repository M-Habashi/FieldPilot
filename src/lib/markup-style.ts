...
export const LINE_STYLE_OPTIONS: Array<{
  value: Exclude<MarkupLineStyle, 'cloud'>;
  label: string;
}> = [
...
export function lineDashArray(style: MarkupLineStyle | undefined, width: number): number[] {
  ...
  switch (style) {
    ...
    case 'dashed-4':
      return [unit * 6, unit * 2];
    case 'dashed-5':
      return [unit * 8, unit * 3, unit * 2, unit * 3];
    case 'dashed-6':
      return [unit * 10, unit * 3];
    default:
      return [];
  }
}

export function svgDashArray(
  style: MarkupLineStyle | undefined,
  width: number,
): string | undefined {
